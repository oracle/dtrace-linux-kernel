		...
		case DW_TAG_member: {
			...
			case NS_NEEDS_SHARING:
				g_hash_table_replace(id_to_module, id,
						     xstrdup("shared_ctf"));
				dw_ctf_trace("Marking bitfield member %s as shared\n", id);
					    ...
					    struct die_override *overrides)
{
	...
	if (ctf_module == NULL) {
		fprintf(stderr, "Internal error: within file %s, module %s, type at DIE offset %lx\n"
			"with ID %s was not already noted by detect_duplicates().\n", file_name,
	...
	if ((strcmp(ctf_module, module_name) != 0) &&
		...
		fprintf(stderr, "Internal error: within file %s, module %s, type at DIE offset %lx\n"
			...
			module_name, (unsigned long) dwarf_dieoffset(die), id, ctf_module);
		fprintf(stderr, "detect_duplicates() is probably buggy.\n");
		exit(1);
	}
				...
				const char *locerrstr)
{
	...
	dw_ctf_trace("    %s: Looking up dependent type at offset %lx for type %s at module %s, file %s\n",
		     locerrstr, (unsigned long) dwarf_dieoffset(type_die),
				     ...
				     enum skip_type *skip, int *replace)
{
	...
	type_ref = lookup_ctf_type(module_name, file_name, die, ctf, locerrstr);
	if (type_ref < 0) {
		*skip = SKIP_ABORT;
		return CTF_ERROR_REPORTED;
	}
					  ...
					  int *replace)
{
	...
	id = ctf_add_sou(ctf, ROOT_TYPE(top_level_type), name);
